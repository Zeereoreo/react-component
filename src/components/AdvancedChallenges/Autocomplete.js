...
export const Autocomplete = () => {
   ...
   */
  const [hasText, setHasText] = useState(false); // 인풋값의 유무
  const [inputValue, setInputValue] = useState(''); // 인풋값의 상태
  const [options, setOptions] = useState(deselectedOptions); // 옵션의 상태
  const [selected, setSeleted] = useState(-1); //인풋의 옵션 선택
...
export const DropDown = ({selected, options, handleComboBox }) => {
  return (
    <DropDownContainer>
      ...
      {options.map((el, idx)=> <li className={idx === selected ? 'selected' : ''} onClick={()=>handleComboBox(el)} key={idx} >{el}</li>)}
    </DropDownContainer>
  );
};